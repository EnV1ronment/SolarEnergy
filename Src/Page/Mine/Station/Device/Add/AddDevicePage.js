...
export default class AddDevicePage extends Component {
    ...
    constructor(props) {
        ...
        const {params} = state;
        const {stationId, stationCode} = params;
        this.state = {
    ...
    _scanQRCodeResult = (event) => {
        ...
        if (this._availableScanArea(event) && event.data && typeof event.data === 'string' && event.data.length) {
            this.setState({
                ...
                navigation.getParam('callback')(event.data);
                navigation.goBack();
                // const data = {
    ...
    render() {
        const {startScanQRCode, visible} = this.state;
        return (
            <View style={styles.container}>
                ...
                <WKPresenter
                    visible={visible}
                    message={WK_T(wkLanguageKeys.continue_add)}
                    ...
                    defaultButtonText={WK_T(wkLanguageKeys.continue)}
                    defaultButtonClick={() => {
                        if (this.state.isTorchOpened) {
                            this.state.isTorchOpened = false;
                        }
                        this.setState({
                            visible: false,
                            startScanQRCode: true,
